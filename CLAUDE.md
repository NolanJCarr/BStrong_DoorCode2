...
- **Expired-window SMS not sending** (`app.py:343`): When a member texts in after the 48-hour PIN change window, the route calls `send_sms` with the expired-ticket message but the SMS is not being delivered. Needs investigation — likely a sender-ID mismatch (international number vs. `B-STRONG` alphanumeric sender) or the `from_number` value from Twilio not matching what `send_sms` expects. Verify by logging the return value of `send_sms` and the `from_number` at that branch.
